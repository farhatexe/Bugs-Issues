namespace ProjectTasks.Presentation.Core.Bootstrap.Component.DataTables
{
    ...
    public class DataTablesBinder : IModelBinder
    {
        ...
        protected virtual List<DataTablesColumn> GetColumns(NameValueCollection collection)
        {
            try
            {
                ...
                for (int i = 0; i < collection.Count; i++)
                {
                    ...
                    if (columnData != null && columnName != null)
                    {
                        ...
                        {
                            OrderNumber = -1,
                            PropertyName = columnData,
                            Orderable = Get<bool>(collection, String.Format(COLUMN_ORDERABLE_FORMATTING, i))
                        });
                    }
            ...
            catch
            {
                ...
                return new List<DataTablesColumn>();
            }
        }
        /// <summary>
        ...
        protected virtual void ParseColumnOrdering(NameValueCollection collection, IEnumerable<DataTablesColumn> columns)
        {
            for (int i = 0; i < collection.Count; i++)
            {
                ...
                if (orderColumn > -1 && orderDirection != null)
                    SetColumnOrdering(columns.ElementAt(orderColumn), i, orderDirection);
            }
        }
        ...
        private void SetColumnOrdering(DataTablesColumn column, int orderNumber, string orderDirection)
        {
            column.OrderNumber = orderNumber;

            if (orderDirection.ToLower().Equals("asc")) column.SortDirection = SortDirection.Ascending;
            else if (orderDirection.ToLower().Equals("desc")) column.SortDirection = SortDirection.Descending;
